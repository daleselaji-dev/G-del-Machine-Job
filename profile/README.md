...
- `website/PORTFOLIO_WEBSITE_BLUEPRINT.md` — central map for the personal website, including public/private information architecture, build workflow and award-winning-site design lessons.
- `resumes/ZHIPU_AI_GAME_OPERATIONS_RESUME_V1.md` — anonymized one-page content draft for the AI overseas-game-operations target.
...
5. Maintain `JOB_REQUIREMENTS_DATABASE.md` as the requirement registry: preserve exact JD wording, separate facts from inference and record why a role was discovered or missed.
6. Use company/role modules only when a target requires detailed line-by-line analysis; keep the main database concise.
7. Resume versions select and rewrite supported evidence for a target role, but they do not replace either database or the source archive.
8. Sensitive personal identifiers are excluded because the repository is public.

## Retrieval rule

For a target role, start with `JOB_REQUIREMENTS_DATABASE.md` and any active company module to identify the actual capability and proof requirements, then retrieve matching evidence from `CAREER_EVIDENCE_DATABASE.md`. Use `SEARCH_INDEX.md` for broader responsibility/industry/tool retrieval, and open the referenced baseline or append-only evidence before asserting a disputed or high-impact fact.