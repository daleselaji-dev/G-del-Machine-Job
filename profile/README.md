...
5. Resume versions select and rewrite evidence for a target role, but they do not replace the database or archive.
6. Sensitive personal identifiers are excluded because the repository is public.

## Retrieval rule

Start with `CAREER_EVIDENCE_DATABASE.md` for resume generation, story selection and gap discovery. Use `SEARCH_INDEX.md` when broader responsibility/industry/tool retrieval is needed, then open the referenced baseline or append-only evidence before asserting a disputed or high-impact fact.